...
bool ReadWavFile(const char *path, AudioSample **buffer, unsigned int *numFrames, unsigned int *numChannels )
{
	char tag[4];
	...
	*numFrames = 0;
	...
	fread(&sz, 4, 1, file);
	if (!sz) return false;
	bufSize = sz * sizeof(AudioSample) / (bitsPerSample >> 3);
	*buffer = (AudioSample*)malloc(bufSize);
...
bool WriteWavHeader(FILE *file, int sampleRate)
{
	short sampleFormat = WAVE_FORMAT_FLOAT;
	short numChannels = 2;
	int byteRate = 2 * sampleRate * sizeof(AudioSample);
	short blockAlign = 2 * sizeof(AudioSample);
	short bitsPerSample = sizeof(AudioSample) << 3;
